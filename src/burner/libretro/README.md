...
There are controversies about whether libretro's patreon and retroarch's GPL license breaks our non-commercial license or not. This is what you should know :

* **"Redistributions may not be sold, nor may they be used in a commercial product or activity."** : By definition, a commercial activity is an activity involving the sale of goods or services. The libretro project does none of that, and it is unclear whether a patreon should be treated as a commercial activity or not when no goods or services are provided in exchange of the donation.
* **"You may not ask for donations to support your work on any project that uses the FB Neo source code."** : This FBNeo port is using libretro code, not the other way around. Nobody involved in writing this port is receiving donations. Interestingly, this term shouldn't even exist if receiving donations was de facto a commercial activity.
* *If* the libretro project was a commercial activity, it would still be unclear how it does affect this port. Our win32 standalone builds use the directx api, which belongs to a commercial company. Using the libretro api wouldn't be different, and there are alternative libretro frontends that don't belong to the libretro project.
* While GPL code can't be mixed with non-commercial code, this port doesn't contain any of the GPL-licensed libretro code.
