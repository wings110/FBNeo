...
### Why are some people saying this libretro port is breaking FBNeo's license ?

This is something i have been reading from time to time, but the libretro port is not breaking FBNeo's license as far as i am aware, some examples :

* *libretro is a commercial activity, so it breaks "Redistributions may not be sold, nor may they be used in a commercial product or activity.".* That's wrong, a commercial activity is an activity involving the sale of goods or services, the libretro project is doing nothing of the sort. Just receiving donations doesn't transform you into a commercial activity, hence why in the first place there are additional terms concerning donations in our license.
* *GPL and non-commercial licenses are incompatibles.* While that's true, the libretro components used by this libretro port are not under GPL license, so there is no problem with this either. 
* *linking GPL software and non-commercial software is producing a derivative work, extending the GPL license to the non-commercial work.* While that might be a *grey zone* in some countries, it's simply not true under European Law, where the libretro buildbots are located. Linking two programs or linking an existing software with your own work does not produce a derivative or extend the coverage of the linked software licence to your own work. This opinion is based on the Directive 91/250/EEC on the legal protection of computer programs (re-codified in Directive 2009/24/EC).
